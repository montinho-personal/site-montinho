...
export function cicloDoLead(b: Base, lead: Lead, opp: Oportunidade | undefined, atividades: Atividade[], experimentais: Experimental[]): Ciclo {
  ...
  const ultimaExperimental = experimentais.map((t) => t.created_at).sort().at(-1);
  const adiado = atividades.filter((a) => a.metadata?.adiado === true).map((a) => a.ocorreu_em).sort().at(-1);
  return cicloDeFollowUps(
    [lead.last_reply_at, opp?.proposal_sent_at, ultimaEtapa, ultimaExperimental, adiado],
    atividades.map((a) => ({ ocorreuEm: a.ocorreu_em, tipo: a.tipo, metadata: a.metadata })),
  );
}
...
export function visaoLead(b: Base, cat: Catalogo, lead: Lead, agora = new Date()): LeadVisao | null {
  ...
  return {
    ...
    ciclo: cicloDoLead(b, lead, opp, atividades, experimentais),
  };
}
