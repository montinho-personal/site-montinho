...
export default function AnalyticsNav({ atual }: { atual: string }) {
  return <nav className="mb-4 flex flex-wrap gap-1">{ITENS.map(([h, n]) => <Link key={h} href={h} className={`rounded-full px-3 py-1 text-xs ${h === atual ? "bg-white text-black" : "border border-white/15 text-zinc-300 hover:bg-white/10"}`}>{n}</Link>)}</nav>;
}
export function Periodo({ atual, base }: { atual: number; base: string }) {
  return <div className="mb-3 flex gap-1 text-xs">{[30, 90, 180, 365].map((d) => <Link key={d} href={`${base}?dias=${d}`} className={`rounded px-2 py-1 ${d === atual ? "bg-white/20" : "text-zinc-400 hover:text-white"}`}>{d} dias</Link>)}</div>;
}
