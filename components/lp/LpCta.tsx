 ...
 *
 * `data-cta-id` é lido pelo HandoffTracker do CRM: o clique sai com página,
 * posição do CTA, gclid e código Ref na mensagem. O evento `lp_cta_click`
 * responde de onde a pessoa clicou; `click_whatsapp` e `generate_lead` já
 * são disparados globalmente e servem de conversão no GTM.
 */
export default function LpCta({ label, message, sub, posicao, lp, grande = true }: { label: string; message: string; sub?: string; posicao: string; lp: string; grande?: boolean }) {
  return (
...
export function LpCtaFixa({ message, lp }: { message: string; lp: string }) {
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-black/90 p-3 backdrop-blur sm:hidden">
      <LpCta label="Falar no WhatsApp" message={message} posicao="fixa" lp={lp} grande={false} />
    </div>
  );
}
